...
#pragma omp parallel for
    for (unsigned block = 0; block < numBlocks; block++)
    {
        ...
        {
            if (block * blockSize + i >= keys.size())
                break;
            uint key = keys[block * blockSize + i];
    ...
    uint numBuckets)
{
    ...
    // TODO
    for (unsigned bucket = 1; bucket < numBuckets; bucket++)
    {
    ...
    const std::vector<uint> &blockHistograms)
{
    ...
    for (unsigned block = 0; block < numBlocks; block++)
    {
        for (unsigned bucket = 0; bucket < numBuckets; bucket++)
        {
            if (block == 0)
            {
                blockExScan[block * numBuckets + bucket] = globalHistoExScan[bucket];
            }
            else
            {
...
{
    // TODO
    std::vector<uint> localOffSets(numBuckets * numBlocks, 0);
#pragma omp parallel for
    for (unsigned block = 0; block < numBlocks; block++)
    {
        for (unsigned i = 0; i < blockSize; i++)
        {
            ...
            uint key = keys[block * blockSize + i];
            uint bucket = (key >> startBit) & (numBuckets - 1);
            uint pos = blockExScan[block * numBuckets + bucket] + localOffSets[block * numBuckets + bucket];
            sorted[pos] = key;
            localOffSets[block * numBuckets + bucket]++;
        }
    }
}
