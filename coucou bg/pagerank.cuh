...
__global__ void device_graph_propagate(
    ...
    for(int idx = index; idx < num_nodes; idx += stride){
        graph_nodes_out[idx] = 1 / ((float) 2 * num_nodes);
        for (int i = graph_indices[idx]; i < graph_indices[idx + 1]; i++) {
            graph_nodes_out[idx] += 0.5 * graph_nodes_in[graph_edges[i]] * inv_edges_per_node[graph_edges[i]];
        }
    }
}
...
double device_graph_iterate(
    ...
    uint* d_graph_edges;
    float* d_node_values_input;
    float* d_gpu_node_values_output;
    ...
    cudaMemcpy(h_gpu_node_values_output, d_gpu_node_values_output, num_nodes * sizeof(float), cudaMemcpyDeviceToHost);
...
{
    return iterations * (nodes * (3 + 3 * edges) * sizeof(uint));
}
